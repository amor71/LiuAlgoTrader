...
CREATE TABLE IF NOT EXISTS algo_run (algo_run_id serial PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS new_trades (trade_id serial PRIMARY KEY,
                                                               algo_run_id integer REFERENCES algo_run(algo_run_id),
                                                                                              symbol text NOT NULL,
                                                                                                          operation trade_operation NOT NULL,
                                                                                                                                    qty integer NOT NULL check (qty > 0), price decimal (8, 2) NOT NULL,
                                                                                                                                                                                               indicators jsonb NOT NULL,
                                                                                                                                                                                                                client_time text, tstamp timestamp DEFAULT current_timestamp);
...
CREATE TABLE IF NOT EXISTS ticker_data (symbol text PRIMARY KEY,
                                                            name text NOT NULL,
                                                                      description text NOT NULL,
                                                                                       tags text[], similar_tickers text[], industry text, sector text, exchange text, short_ratio float, create_tstamp timestamp DEFAULT current_timestamp,
...
CREATE TABLE IF NOT EXISTS trending_tickers (trending_id serial PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS stock_ohlc (symbol_id serial PRIMARY KEY,
                                                                symbol text NOT NULL,
...
CREATE TABLE IF NOT EXISTS gain_loss (gain_loss_id serial PRIMARY KEY,
                                                                  symbol text NOT NULL,
...
ALTER TABLE new_trades
ALTER COLUMN algo_run_id
SET NOT NULL;


CREATE TABLE IF NOT EXISTS trade_analysis (trade_analysis_id serial PRIMARY KEY,
                                                                            symbol text NOT NULL,
                                                                                        algo_run_id integer NOT NULL REFERENCES algo_run(algo_run_id),
...
CREATE TABLE IF NOT EXISTS portfolio (portfolio_entry_id serial PRIMARY KEY,
                                                                        portfolio_id text NOT NULL,
                                                                                          symbol text NOT NULL,
                                                                                                      rank decimal (14, 10) NOT NULL,
                                                                                                                            atr decimal (6, 2) NOT NULL,
                                                                                                                                               qty int NOT NULL,
                                                                                                                                                       tstamp timestamp with time zone DEFAULT current_timestamp,
