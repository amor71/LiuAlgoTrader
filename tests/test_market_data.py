...
async def test_get_trading_day():
    ...
    d1 = await get_trading_day(now=now, offset=1)
    print(d1)
    _df = alpaca.get_equity_trading_days(d1, now.date())

    if len(_df.index) < 1 or len(_df.index) > 2:
        raise AssertionError("expected offset 1")

    d1 = await get_trading_day(now=now, offset=40)
    print(d1)
    _df = alpaca.get_equity_trading_days(d1, now.date())
    ...
    d1 = await get_trading_day(now=now, offset=100)
    print(d1)
    _df = alpaca.get_equity_trading_days(d1, now.date())
    ...
    d1 = await get_trading_day(now=now, offset=300)
    print(d1)
    _df = alpaca.get_equity_trading_days(d1, now.date())
    ...
    d1 = await get_trading_day(now=now, offset=1000)
    print(d1)
    _df = alpaca.get_equity_trading_days(d1, now.date())
...
async def test_load_sp500_data():
    ...
    d1 = datetime.combine(
        await get_trading_day(now=today.date(), offset=200), ttime.min
    )

    print(f"start:{d1}, end:{today.date()}")
    t0 = time.time()
    dl.pre_fetch(symbols=sp500_symbols, start=d1, end=today)
    t = time.time() - t0
